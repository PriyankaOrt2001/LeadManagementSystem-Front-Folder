function SaveFormData() {
	...
	if (PlanName == "") {
		...
		$('#ErrorForPlanName').html('Please enter Plan Name.');
		$('#TxtPlanName').focus();
		return;
	} else if (!onlyText.test(PlanName)) {
		$('.help-block').html('');
		$('#PlanNameDIV').addClass('has-error');
		$('#ErrorForPlanName').html('Please enter valid Plan Name.');
		$('#TxtPlanName').focus();
		return;
	}
	else if (PlanPrice == "") {
		...
		$('#ErrorForPlanPrice').html('Please enter Plan Price.');
		$('#TxtPlanPrice').focus();
		return;
	} else if (PlanPrice == "0") {
		$('.help-block').html('');
		$('#PlanPriceDIV').addClass('has-error');
		$('#ErrorForPlanPrice').html('Please enter valid Plan Price.');
		$('#TxtPlanPrice').focus();
		return;
	}
