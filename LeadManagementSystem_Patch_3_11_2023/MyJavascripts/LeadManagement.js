...
function IsFavLead(id, IsFav) {
	...
	// Find the 'a' tag within the active page element
	var aTag = activePageElement.find('a');

	// Get the value of the 'data-dt-idx' attribute
	var dataDtIdxValue = aTag.data('dt-idx');

	console.log(dataDtIdxValue);
				...
				$.ajax({
					...
					success: function (return_Data) {
						if (return_Data.n == 5) {
							...
							refreshDataTable();
							bootbox.alert(return_Data.msg);
							$('li:has(a[data-dt-idx="1"])').removeClass('active');
							$('li:has(a[data-dt-idx="2"])').addClass('active');
							debugger;
						}
						else {
							refreshDataTable();
...
function changeLeadStatus(id, Status) {
	...
	// Get the value attribute's value
...
$(function () {
	$("#btnDownloadReport").click(function () {
		debugger;
		var table = document.getElementById("TableLead"); // Replace "myTable" with your table id
...
function nevigateToEditContactDetails() {
	...
	if (CompanyName == "") {
		...
		$('.help-block').html('');
		$('#ClientNameDIV').addClass('has-error');
		$('#ErrorForClientName').html('Please enter valid Client Name.');
		$('#TxtClientName').focus();
		return;
	}
	else if (Source == "") {
